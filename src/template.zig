...
test "load templates" {
    ...
    initDynamic(a, "src/routing/fallback_html/");
    defer raze(a);

    //try std.testing.expectEqual(3, builtin.len);
    for (builtin) |bi| {
        if (std.mem.eql(u8, bi.name, "routing/fallback_html/index.html")) {
            try std.testing.expectEqualStrings("routing/fallback_html/index.html", bi.name);
...
test findTemplate {
    const tmpl = findTemplate("routing/fallback_html/index.html");
    try std.testing.expectEqualStrings("routing/fallback_html/index.html", tmpl.name);
}
